...
int esweepExpr(ClientData clientdata, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[]) {
	...
	CHECK_NUM_ARGS(objc == 4, "objVarName operator \"object or scalar\"");
	...
	if (objv[3]->typePtr == &tclEsweepObjType) {
		...
		if (Tcl_GetDoubleFromObj(NULL, objv[3], &vald)==TCL_ERROR) {
			Tcl_SetResult(interp, "Parameter 3 neither an esweep object nor a scalae", TCL_STATIC);
			return TCL_ERROR;
		}
...
int esweepClip(ClientData clientdata, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[]) {
	esweep_object *a=NULL, *b=NULL;
	Tcl_Obj *tclObj=NULL;
	const char *opts[] = {"-obj", "-clip", "-above", NULL};
	int optMask[] = {1, 1, 0}; // necessary options
	enum optIdx {objIdx, clipIdx, aboveIdx};
	int obji;
	int index;
	...
	CHECK_NUM_ARGS(objc == 5 || objc == 6, "-obj objVarName -clip \"object or scalar\" -above");
	...
	for (obji=1; obji < objc; obji++) {
		...
		switch (index) {
			case objIdx:
				obji++;
				CHECK_ESWEEP_OBJECT2(obji, tclObj, a);
				break;
			case clipIdx:
				obji++;
        /* this parameter can be an esweep object*/
        if (objv[obji]->typePtr == &tclEsweepObjType) {
          b=(esweep_object*) objv[obji]->internalRep.otherValuePtr;
        } else { // but it may also be a scalar
          scalar=obji; // remember that this parameter is a scalar, we need to convert it to an esweep object
        }
				break;
			case aboveIdx:
				above=1;
  ...
  if (scalar) {
    if (Tcl_GetDoubleFromObj(NULL, objv[scalar], &vald)==TCL_ERROR) {
      Tcl_SetResult(interp, "Option -clip neither an esweep object nor a scalar", TCL_STATIC);
      return TCL_ERROR;
    }
    /* convert it into an esweep object */
    esweep_type(a, &type);
	...
	DUPLICATE_WHEN_SHARED(tclObj, obj);
  if (above) {
    ESWEEP_TCL_ASSERT(esweep_clipUpper(a, b) == ERR_OK);
  } else {
    ESWEEP_TCL_ASSERT(esweep_clipLower(a, b) == ERR_OK);
  }
