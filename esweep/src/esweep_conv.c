...
int esweep_compress(esweep_object *obj, int32_t factor) {
	Complex *cpx;
	Polar *polar;
  Wave *wave;
	int i, j;
	...
	ESWEEP_OBJ_ISVALID(obj, ERR_OBJ_NOT_VALID);
	ESWEEP_OBJ_NOTEMPTY(obj, ERR_EMPTY_OBJECT);
  ESWEEP_ASSERT(factor > 0, ERR_BAD_PARAMETER);

	switch (obj->type) {
    case WAVE:
      wave=(Wave*) obj->data;
      for (i=0, j=0; j < obj->size; i++, j+=factor) { // compress data
        wave[i]=wave[j];
      }
      for (; i < obj->size;i++) { // set all other to 0
        wave[i]=0.0;
      }
      break;
    case COMPLEX:
      cpx=(Complex*) obj->data;
      for (i=0, j=0; j < obj->size; i++, j+=factor) { // compress data
        cpx[i].real=cpx[j].real;
        cpx[i].imag=cpx[j].imag;
      }
      for (; i < obj->size;i++) { // set all other to 0
        cpx[i].real=cpx[i].imag=0.0;
      }
      break;
    case POLAR:
      polar=(Polar*) obj->data;
      for (i=0, j=0; j < obj->size/2-1; i++, j+=factor) { // compress data
        polar[i].abs=polar[j].abs;
        polar[i].arg=polar[j].arg;
      }
      for (; i < obj->size/2-1; i++) { // set all other to 0
        polar[i].abs=polar[i].arg=0.0;
      }
      for (i=1; i < obj->size/2-1; i++) { /* restore hermitian redundancy */
